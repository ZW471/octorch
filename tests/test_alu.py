...
class TestBasicALU:
    ...
    def test_alu_set_basic(self, fresh_state):
        """8XY0 - Set VX = VY."""
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0x42))
        state = state.replace(V=put(state.V, 2, 0x99))
    ...
    def test_alu_or_basic(self, fresh_state):
        """8XY1 - OR operation."""
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0xF0))
        state = state.replace(V=put(state.V, 2, 0x0F))
    ...
    def test_alu_and_basic(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0xF0))
        state = state.replace(V=put(state.V, 2, 0xF1))
    ...
    def test_alu_xor_basic(self, fresh_state):
        """8XY3 - XOR operation."""
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0xFF))
        state = state.replace(V=put(state.V, 2, 0xF0))
    ...
    def test_alu_xor_same(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 3, 0xAA))
        state = state.replace(V=put(state.V, 4, 0xAA))
...
class TestALUArithmetic:
    ...
    def test_alu_add_no_carry(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0x10))
        state = state.replace(V=put(state.V, 2, 0x20))
    ...
    def test_alu_add_with_carry(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0xFF))
        state = state.replace(V=put(state.V, 2, 0x01))
    ...
    def test_alu_sub_xy_no_borrow(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0x30))
        state = state.replace(V=put(state.V, 2, 0x10))
    ...
    def test_alu_sub_xy_with_borrow(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 3, 0x10))
        state = state.replace(V=put(state.V, 4, 0x30))
    ...
    def test_alu_sub_yx_no_borrow(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 1, 0x10))
        state = state.replace(V=put(state.V, 2, 0x30))
...
class TestALUShifts:
    ...
    def test_shift_right_modern_even(self, modern_state):
        ...
        state = modern_state.replace(V=put(modern_state.V, 1, 0x04))
        state = state.replace(V=put(state.V, 2, 0xFF))  # Should be ignored
    ...
    def test_shift_right_modern_odd(self, modern_state):
        ...
        state = modern_state.replace(V=put(modern_state.V, 3, 0x05))
        state = state.replace(V=put(state.V, 4, 0xFF))  # Should be ignored
    ...
    def test_shift_right_legacy(self, legacy_state):
        ...
        state = legacy_state.replace(V=put(legacy_state.V, 5, 0x08))  # Should be ignored
        state = state.replace(V=put(state.V, 6, 0x03))  # 00000011
    ...
    def test_shift_left_modern_overflow(self, modern_state):
        ...
        state = modern_state.replace(V=put(modern_state.V, 3, 0x81))  # 10000001
        state = state.replace(V=put(state.V, 4, 0xFF))  # Should be ignored
    ...
    def test_shift_mode_comparison(self):
        ...
        state_modern = state_modern.replace(V=put(state_modern.V, 1, 0x08))  # V1 = 8
        state_modern = state_modern.replace(V=put(state_modern.V, 2, 0x03))  # V2 = 3
        state_modern = execute(state_modern, 0x8126)

        # Legacy: V1 = V2 >> 1 (uses V2)
        state_legacy = create_state().replace(modern_mode=False)
        state_legacy = state_legacy.replace(V=put(state_legacy.V, 1, 0x08))  # V1 = 8
        state_legacy = state_legacy.replace(V=put(state_legacy.V, 2, 0x03))  # V2 = 3
...
class TestALUEdgeCases:
    ...
    def test_alu_self_operations(self, fresh_state):
        ...
        state = fresh_state.replace(V=put(fresh_state.V, 5, 0xAA))
        ...
        state = state.replace(V=put(state.V, 5, 0x80))
    ...
    def test_vf_register_operations(self, fresh_state):
        ...
        state = fresh_state
        state = state.replace(V=put(state.V, 15, 0x42))  # VF = 0x42
        state = state.replace(V=put(state.V, 1, 0x10))
